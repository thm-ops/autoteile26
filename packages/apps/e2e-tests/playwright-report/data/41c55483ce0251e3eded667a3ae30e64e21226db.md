...
- Name: cart.spec.ts >> Warenkorb-Funktionalität >> sollte Gesamtsumme anzeigen
- Location: tests\cart.spec.ts:130:7
...
Locator: locator('text=Gesamtsumme')
Expected: visible
Error: strict mode violation: locator('text=Gesamtsumme') resolved to 2 elements:
    1) <span>Gesamtsumme</span> aka getByRole('main').getByText('Gesamtsumme')
    2) <span>Gesamtsumme</span> aka getByText('Gesamtsumme').nth(1)

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('text=Gesamtsumme')
...
- generic [active] [ref=e1]:
    - banner [ref=e2]:
        - generic [ref=e3]:
            - link "MeinShop" [ref=e4]:
                - /url: /
            - generic [ref=e5]:
                - link "Startseite" [ref=e6]:
                    - /url: /
    ...
    - generic [ref=e57]:
        ...
        - generic [ref=e86]:
            ...
            - link "Warenkorb-Seite ansehen" [ref=e103]:
                - /url: /cart
```
...
```ts
  43  |     // Seite sollte Warenkorb-Überschrift anzeigen
  44  |     const heading = page.locator('h1:has-text("Warenkorb")');
  45  |     await expect(heading).toBeVisible();
  46  |   });
  47  |
  48  |   test('sollte Artikel-Menge mit + Button erhöhen', async ({ page }) => {
  49  |     await page.goto('/');
  50  |     await page.waitForLoadState('networkidle');
  51  |
  52  |     const addButton = page.locator('button:has-text("In den Warenkorb")').first();
